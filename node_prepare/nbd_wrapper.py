...
def find_available():
    devices = []
    busy = []
    for x in os.popen("cat /proc/partitions | grep nbd | "
...
def connect(image_file, read_only=True):
    ...
    os.popen('sudo qemu-nbd -c /dev/%s %s %s' % (dev, read_only, image_file))
...
def disconnect(dev=None):
    ...
    if dev is None:
        ...
        os.popen('sudo qemu-nbd -d /dev/%s' % dev)
...
def mount(dev, partition=1, path=None):
    ...
    os.popen('sudo mount %s %s' % (full_dev_path, path))
...
def find_mount(dev=None):
    if dev is None:
        ...
        sys_mount = os.popen('mount | grep %s' %
                             dev).readline().strip().split(' ')
        if len(sys_mount) > 1:
            mount = {
                'dev': sys_mount[0],
                'mount': sys_mount[2],
...
def umount(dev=None):
    ...
    if dev is None:
        for x in m:
            os.popen('sudo umount %s' % x['mount'])
    elif m is not None:
        os.popen('sudo umount %s' % m['mount'])
